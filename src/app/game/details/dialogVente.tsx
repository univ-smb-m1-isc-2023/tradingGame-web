...
import { Ordre, ordreAchat, ordreVente } from "./ordrefunctions";
 
export const DialogVente: React.FC<Ordre> = ({player,game,openValue,symbol}) => {
  ...
  const achat = () => {
    handleOpen();
    ordreVente(player, game, isChecked, price, quantity,symbol);
  };
  ...
  return (
    <>
      <Button
  className="px-8 py-8 mr-8 bg-red-500 text-white rounded-md focus:outline-none w-300"
        onClick={handleOpen}
        variant="gradient"
        placeholder={null}
      ...
      <Dialog open={open} handler={handleOpen} placeholder={null} onPointerEnterCapture={null} onPointerLeaveCapture={null}>
        <div className="fixed inset-0 flex items-center justify-center bg-white bg-opacity-80"> {/* Ajoutez la classe bg-white pour définir un arrière-plan blanc */}
          <div className="border border-black p-4 rounded-lg bg-white">
            <DialogHeader placeholder={null} onPointerEnterCapture={null} onPointerLeaveCapture={null}>Ordre de vente</DialogHeader>
            <DialogBody placeholder={null} onPointerEnterCapture={null} onPointerLeaveCapture={null}>
              ...
              <div className=" mt-2 mb-4">
                ...
                <input
                  ...
                  defaultValue={openValue.toLocaleString()}
